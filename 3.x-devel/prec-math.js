...
(function (window, undefined){
  ...
  function tan(z, nprec){
    ...
    var sin = sinr(a2, nprec+2);
    var sinh = sinhr(b2, nprec+2);
    //alert(len(sin));
    var coscosh = addr(cosr(a2, nprec+4+Math.max(len(sin), len(sinh))),
                       coshr(b2, nprec+4+Math.max(len(sin), len(sinh))));
    return N(divr(sinr(a2, nprec+2), coscosh, nprec),
             divr(sinh, coscosh, nprec));
  }
