/***** Perfectly Precise Math Library 1.4 *****/
...
function arg(z){
  ...
  return N(atan2r(getB(z), getA(z)), 0);
}
...
function sqrt(z){
  ...
  var absz = getA(abs(z));
  ra = sqrtr(divr(addr(a, absz), 2, prec+2));
  rb = sgno(b) + sqrtr(divr(addr(negr(a), absz), 2, prec+2));
...
function pi(nprec){
  return N(pir(Number(getA(nprec))), 0);
}

function e(nprec){
  return N(er(Number(getA(nprec))), 0);
}

function phi(nprec){
  return N(phir(Number(getA(nprec))), 0);
}
