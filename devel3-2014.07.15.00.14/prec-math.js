...
(function (win, udf){
  ...
  function div(a, b, p){
    ...
    var decA = decp(a)?declen(a):0;
    var decB = decp(b)?declen(b):0;
  ...
  function exp(a, p){
    ...
    if (intp(a)){
      var an = num(a);
      if (an == 1)exp = e(p);
      else if (an < 100)exp = pow(e(p+1+(an-1)*(siz(a)+1)), a, p+1);
      else exp = expTaylorFrac(a, p+2);
    } else {
      var fl = trn(a);
      var flLen = fl.length;
      fl = num(fl);
      ...
      if (fl == 0){
        if (declen <= 30)exp = expTaylorFrac(a, p);
        else exp = expTaylorTerms(a, p);
      } else {
        var expfl;
        if (fl == 1)expfl = e(p+2);
        else if (fl < 100)expfl = pow(e(p+1+(fl-1)*(flLen+1)), str(fl), p+1);
        else expfl = expTaylorFrac(str(fl), p+2);
        
        if (declen <= 30){
          exp = mul(expfl, expTaylorFrac(a, p+2+siz(expfl)), p);
        } else {
          exp = mul(expfl, expTaylorTerms(a, p+2+siz(expfl)), p);
        }
      }
    }
    
    if (sign)return div("1", exp, p-2);
    return exp;
  }
  ...
  function ln(a, p){
    ...
    if (twos != "0"){
      if (twos == fives){
        ...
        ln = add(lnsmall, mul(twos, l10), p);
      } else {
        if (fives != "0"){
          var both = ln2and5(p+2+Math.max(siz(twos), siz(fives)));
          var l2 = both[0];
          var l5 = both[1];
          ln = add(add(lnsmall, mul(twos, l2)),
                    mul(fives, l5),
                    p);
        } else {
          var l2 = ln2(p+2+siz(twos));
          ln = add(lnsmall, mul(twos, l2), p);
        }
      }
    } else {
      if (fives != "0"){
        var l5 = ln5(p+2+siz(fives));
        ln = add(lnsmall, mul(fives, l5), p);
      } else {
        ln = rnd(lnsmall, p);
      }
    }
  ...
  */
  ...
  function fact(a, p){
    if (decp(a) || negp(a)){
      err(fact, "a must be a positive integer");
    }
    if (p == -inf)return "0";
    a = num(a);
  ...
  function frac(a, b, p){
    ...
    frac.lastNums = [n, pn, qn];
    
    return div(pn, qn, p);
  }
  
  sfrac.lastNums = [0, "0", "1"]; // [n, pn, qn]
  function sfrac(a, p){
    if (p == udf)p = prec;
    if (p == -inf)return "0";
  ...
  var logfn = function (subj, data){};
  
  function log(subj){
    logfn(subj + ": ", $.apl($.stf, $.sli(arguments, 1)));
  }
