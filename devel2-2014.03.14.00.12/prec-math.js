...
(function (win, udef){
  ...
  function remdot(a){
    var dot = pos(".", a);
    if (dot == -1)return a;
    return slc(a, 0, dot) + slc(a, dot+1);
  }
  ...
  function decrem(a){
    var dot = pos(".", a);
    if (dot == -1)return [0, a];
    return [len(a)-1-dot, triml(slc(a, 0, dot) + slc(a, dot+1))];
  }
  ...
  function triml(a){
    for (var i = 0; i < len(a); i++){
      if (a[i] != '0'){
        if (a[i] == '.')return slc(a, i-1);
        if (i != 0)return slc(a, i);
        return a;
      }
    }
    return "0";
  ...
  function trimr(a){
    for (var i = len(a)-1; i >= 0; i--){
      if (a[i] != '0'){
        if (a[i] == '.')return slc(a, 0, i);
        if (i != len(a)-1)return slc(a, 0, i+1);
        return a;
      }
    }
    return "0";
  ...
  function rndchr(a, p){
    var pos = rndpos(a, p);
    return (udefp(a[pos]) || a[pos] == '-')?"0":a[pos];
  }
  ...
  function chke(a){
    ...
    var front = slc(a, 0, pos);
    var sign = a[pos+1];
    var back = num(slc(a, pos+2));
  ...
  function left(a, n){ // 32.44 -> 3.244
    ...
    if (zeros >= 0){
      ...
      if (adot == alen){
        a = slc(a, 0, adot-n) + "." + slc(a, adot-n);
      } else {
        a = slc(a, 0, adot-n) + "." + slc(a, adot-n, adot) + slc(a, adot+1);
      }
    }
  ...
  function right(a, n){ // 32.44 -> 324.4
    ...
    if (adot != -1){
      ...
      if (zeros >= 0){
        a += nof("0", zeros);
        a = slc(a, 0, adot) + slc(a, adot+1);
      } else {
        a = slc(a, 0, adot) + slc(a, adot+1, adot+1+n) + "." + slc(a, adot+1+n);
      }
    } else {
      a += nof("0", n);
    }
  ...
  function sub(a, b, p){
    ...
    return udefp(p)?diff:rnd(diff, p);
  }
  ...
  function mul(a, b, p){
    ...
    if (dot != -1){
      dec += len(a)-1-dot;
      a = triml(slc(a, 0, dot) + slc(a, dot+1));
    }
    dot = pos(".", b);
    if (dot != -1){
      dec += len(b)-1-dot;
      b = triml(slc(b, 0, dot) + slc(b, dot+1));
    }
    ...
    return udefp(p)?prod:rnd(prod, p);
  }
  ...
  function mulLong(a, b){
    ...
    for (var i = len(b); i > 0; i -= 7){
      ...
      for (var j = len(a); j > 0; j -= 7){
        ...
        ln = len(small);
        if (ln > 7){
          curr = slc(small, ln-7, ln) + curr;
          carry = num(slc(small, 0, ln-7));
        } else {
          curr = small + curr;
  ...
  function mulKarat(a, b){
    ...
    if (alen != blen){
      ...
      */
      ...
      var a1 = slc(a, 0, alen-m);
      var a0 = triml(slc(a, alen-m, alen));
    ...
    */
    ...
    var a1 = slc(a, 0, alen-m);
    var a0 = triml(slc(a, alen-m, alen));
    var b1 = slc(b, 0, blen-m);
    var b0 = triml(slc(b, blen-m, blen));
  ...
  function ceiPos(a, p){
    ...
    if (p == 0 || udefp(p)){
      if (dot == -1)return a;
      return add1(slc(a, 0, dot));
    }
    if (p < 0){
      if (dot == -1)dot = len(a);
      if (dot+p <= 0)return "1" + nof("0", -p);
      return add1(slc(a, 0, dot+p)) + nof("0", -p);
    }
    if (dot == -1 || dot+p+1 >= len(a))return a;
    return trimr(add1(slc(a, 0, dot+p+1)));
  }
  ...
  function flrPos(a, p){
    ...
    if (p < 0){
      if (dot == -1)dot = len(a);
      if (dot+p <= 0)return "0";
      return slc(a, 0, dot+p) + nof("0", -p);
    }
    if (dot == -1 || dot+p+1 >= len(a))return a;
    return trimr(slc(a, 0, dot+p+1));
  }
  ...
  function add1(a){
    ...
    for (var i = len(a)-1; i >= 0; i--){
      if (a[i] == '.')continue;
      if (a[i] != '9')
        return slc(a, 0, i) + (num(a[i])+1) + nof("0", adot-1-i);
    }
    return "1" + nof("0", adot);
  }
  ...
  function powPos(a, b, p){
    if (intp(b)){
      ...
      if (intp(a) || udefp(p))return powExact(a, num(b), p);
  ...
  */
  ...
  function fact(a, p){
    ...
    var fact = factDiv(a);
    return udefp(p)?fact:rnd(fact, p);
  }
  ...
  function frac(a, b, p){
    ...
    var p0 = "1";
    var p1 = a(0);
    if (nullp(p1))return fin(0, "0", "1");
    var q0 = "0";
    var q1 = "1";
    var bn1 = b(1);
    if (nullp(bn1))return fin(0, p1, q1);
    var prod = bn1;
    var pn, qn, an, bn;
    for (var n = 1; true; n++){
      an = a(n); bn = bn1;
      if (nullp(an) || nullp(bn))return fin(n-1, p1, q1);
      bn1 = b(n+1);
      pn = add(mul(an, p1), mul(bn, p0));
      qn = add(mul(an, q1), mul(bn, q0));
      if (!nullp(bn1))prod = mul(prod, bn1);
  ...
  function sfrac(a, p){
    ...
    for (var n = 1; true; n++){
      an = a(n);
      if (nullp(an))return fin(n-1, p1, q1);
      pn = add(mul(an, p1), p0);
      qn = add(mul(an, q1), q0);
  ...
  function log(subj){
    logger(subj + ": ", $.apply($.strf, $.slc(arguments, 1)));
  }
  
  function logfn(f){
    if (udefp(f))return logger;
    return logger = f;
  }
  
  $.errfn(function (subj, data){
    logger(subj, data);
  });
